...
class LegacyServiceSpec extends RatpackGroovyDslSpec {
  ...
  class LegacyRecordingService implements ratpack.core.server.Service {
    ...
    @Override
    void onStart(ratpack.core.server.StartEvent event) throws Exception {
      events << "${prefix ? prefix + "-" : ""}start".toString()
      if (onStart) {
        Operation.of { onStart.run() }.then()
      }
    ...
    @Override
    void onStop(ratpack.core.server.StopEvent event) throws Exception {
      events << "${prefix ? prefix + "-" : ""}stop".toString()
      if (onStop) {
        Operation.of { onStop.run() }.then()
      }
