...
public class DefaultHttpClient implements HttpClientInternal {
  ...
  public Promise<ReceivedResponse> request(URI uri, final Action<? super RequestSpec> requestConfigurer) {
    return Promise.async(downstream -> new ContentAggregatingRequestAction(requestConfigurer, uri, this, Execution.current(), 0).connect(downstream));
  }

  @Override
  public Promise<StreamedResponse> requestStream(URI uri, Action<? super RequestSpec> requestConfigurer) {
    return Promise.async(downstream -> new ContentStreamingRequestAction(requestConfigurer, uri, this, Execution.current(), 0).connect(downstream));
  }
