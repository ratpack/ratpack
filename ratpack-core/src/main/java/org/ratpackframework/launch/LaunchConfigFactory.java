...
/**
 * Static factory methods for creating {@link LaunchConfig} objects using various strategies.
 * <p>
 * Designed to be used to construct a launch config via a {@link Properties} instance.
 * <p>
 * Most methods eventually delegate to {@link #createWithBaseDir(ClassLoader, java.io.File, java.util.Properties)}.
 * <p>
 * When things go wrong, a {@link LaunchException} will be thrown.
 */
public abstract class LaunchConfigFactory {
  ...
  /**
   * Delegates to {@link #createFromGlobalProperties(ClassLoader, String, java.util.Properties, java.util.Properties)}, extracting the “propertyPrefix” from the “globalProperties”.
   * <p>
   * Determines the “propertyPrefix” value and delegates to {@link #createFromGlobalProperties(ClassLoader, String, java.util.Properties, java.util.Properties)}.
   * The value is determined by: {@code globalProperties.getProperty(SYSPROP_PREFIX_PROPERTY, SYSPROP_PREFIX_DEFAULT)}
   *
  ...
  /**
   * Delegates to {@link #createFromProperties(ClassLoader, java.util.Properties, java.util.Properties)} after extracting the “prefixed” properties from the global properties.
   * <p>
   * The properties of {@code globalProperties} that begin with {@code propertyPrefix} are extracted into a new property set,
   * with the property names having the prefix removed.
   * This becomes the {@code overrideProperties} for the delegation to {@link #createFromProperties(ClassLoader, java.util.Properties, java.util.Properties)}.
   *
   * @param classLoader The classloader to use to find the properties file
  ...
  /**
   * Delegates to {@link #createFromFile(ClassLoader, java.io.File, java.io.File, java.util.Properties, java.util.Properties)}, after trying to find the config properties file.
   * <p>
   * The {@code overrideProperties} are queried for the {@link #CONFIG_RESOURCE_PROPERTY} (defaulting to {@link #CONFIG_RESOURCE_DEFAULT}).
   * The {@code classLoader} is asked for the classpath resource with this name.
   * <p>
   * If the resource DOES NOT exist, {@link #createFromFile(ClassLoader, java.io.File, java.io.File, java.util.Properties, java.util.Properties)} is called with
   * the current JVM user dir as the {@code baseDir} and {@code null} as the {@code configFile}.
   * <p>
   * If the resource DOES exist, {@link #createFromFile(ClassLoader, java.io.File, java.io.File, java.util.Properties, java.util.Properties)} is called with
   * that file resource as the {@code configFile}, and its parent directory as the {@code baseDir}.
   *
   * @param classLoader The classloader to use to find the properties file
  ...
  /**
   * Delegates to {@link #createWithBaseDir(ClassLoader, java.io.File, java.util.Properties)}, after merging the properties from {@code configFile} and {@code overrideProperties}.
   * <p>
   * If {@code configFile} exists and is not null, it is read as a properties file.
   * It is then merged with {@code defaultProperties} & {@code overrideProperties}.
   * The default properties are overridden by values in the properties file, while the override properties will override these properties.
   *
   * @param classLoader The classloader to use to find the properties file
  ...
  public final class Property {
     ...
     * <p>
     * This class MUST implement {@link HandlerFactory} and have a public no-arg constructor.
     * <p>
