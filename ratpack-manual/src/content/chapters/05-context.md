...
The [`getPathTokens()`](api/ratpack/handling/ReadOnlyContext.html#getPathTokens\(\)) method on a context is literally shorthand for `get(PathBinding.class).getPathTokens()` on the same context.
...
In the above example, the context's [`file()`](api/ratpack/handling/ReadOnlyContext.html#file\(java.lang.String\)) method is being called to retrieve a `java.io.File` instance for the provided path.
