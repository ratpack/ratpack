...
public class RedisSessionStore implements SessionStore {
  ...
  public Operation store(AsciiString sessionId, ByteBuf sessionData) {
    ...
    return Operation.of(() -> {
      RedisFuture<String> setResult = connection.set(config.getSessionKeyPrefix() + sessionId, sessionData.toString(Charsets.ISO_8859_1));
  ...
  public Promise<ByteBuf> load(AsciiString sessionId) {
    ...
    return sessionStrPromise.map(sessionStr -> {
      ...
      return Unpooled.copiedBuffer(sessionStr, Charsets.ISO_8859_1);
    });
  }
